...
typedef CF_ENUM(int32_t, CiteprocRsErrorCode) {
  ...
  CiteprocRsErrorCodeBufferOps = 6,
  CiteprocRsErrorCodeNullByte = 7,
  CiteprocRsErrorCodeSerdeJson = 8,
};
 ...
 */
typedef void (*CiteprocRsWriteCallback)(void *user_data, const uint8_t *src, uintptr_t src_len);

/**
 * Should clear the buffer in the structure referenced by user_data.
 */
typedef void (*CiteprocRsClearCallback)(void *user_data);
 ...
 */
typedef struct CiteprocRsBufferOps {
  CiteprocRsWriteCallback write;
  CiteprocRsClearCallback clear;
} CiteprocRsBufferOps;
...
typedef struct CiteprocRsInitOptions {
  ...
  struct CiteprocRsBufferOps buffer_ops;
...
CiteprocRsErrorCode citeproc_rs_driver_format_bibliography(struct CiteprocRsDriver *driver,
 ...
 */
CiteprocRsErrorCode citeproc_rs_driver_preview_reference(struct CiteprocRsDriver *driver,
                                                         const char *ref_json,
                                                         uintptr_t ref_json_len,
                                                         void *user_buf) CF_SWIFT_NAME(citeproc_rs_driver_preview_reference(driver:ref_json:ref_json_len:user_buf:));
 ...
 * Returns an error code indicative of what the LAST_ERROR will contain when checked.
 */
CiteprocRsErrorCode citeproc_rs_driver_insert_reference(struct CiteprocRsDriver *driver,
                                                        const char *ref_json,
 ...
 */
CiteprocRsErrorCode citeproc_rs_last_error_utf8(struct CiteprocRsBufferOps buffer_ops,
...
#define CiteprocRsMANAGED_BUFFER_OPS (CiteprocRsBufferOps){ .write = citeproc_rs_cstring_write, .clear = citeproc_rs_cstring_clear }
