...
"#, name=name, ty=ty)?;
    for (cp, string) in lookup.iter() {
        ...
        if let Some(c) = cp.scalar().and_then(|x| if self.scalar { Some(x) } else { None }) {
            write!(self.wtr, "        {:?} => {:?},\n", c, string)?;
        } else {
            write!(self.wtr, "        {} => {:?},\n", cp.value(), string)?;
        }
    }
    write!(self.wtr, r#"        _ => panic!("unknown codepoint in {name} lookup function")"#, name=name)?;
    write!(self.wtr, r#"
    }}
}}
"#)?;
...
pub fn build_superscript_trie() -> Result<()> {
    ...
    let mut writer = Writer { wtr: File::create(&path)?, scalar: true, };
    writer.header()?;
